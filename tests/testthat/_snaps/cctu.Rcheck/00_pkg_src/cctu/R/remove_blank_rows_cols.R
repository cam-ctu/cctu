...
#' parent environment, or if not simply return a modified version of \code{df}
...
#' @param which one of \code{"rows"}, \code{"cols"}, or \code{"both"}. Default
#'  is \code{"both"} to removing both empty rows and empty columns.
#'
#' @return this removes columns and rows that are totally
#' empty (either "" or NA).
#' @export
...
remove_blank_rows_cols <- function(df, convert = TRUE,
  ...
  if (convert) {
    ...
    if (verbose) {
      message(arg_name_df, " modified with blanks removed.")
    }
    invisible(df)
  } else {
    df
