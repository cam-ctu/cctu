...
#' @param dlu Data frame of DLU, see \code{\link{tidy_dlu}} for the
#' requirements of DLU.
#' @param clu Data frame of CLU, see details for the requirements of CLU.
#' @param date_format Date format that will be tried by
...
#' @param clean_names Convert variable name to lower case (default), this will
#'  also change the values in the DLU as well. See \code{\link{clean_names}}
#'  for details.
#' @param rm_empty Remove empty \code{"rows"}, \code{"cols"}, or
#' \code{"both"} (default),
...
#' @param check_catvar Check values of the category variable (defined in the
...
#' }
#' }
#' @seealso \code{\link{var_lab}} \code{\link{val_lab}} \code{\link{tidy_dlu}}
...
apply_macro_dict <- function(data,
  ...
  if (!all(dlu_var_list %in% names(dlu))) {
    stop(
      "Variable ", paste(setdiff(dlu_var_list, names(dlu)), collapse = ", "),
      " not found in the dlu data."
    )
  }

  if (!all(c(
    "shortcode", "description", "type", "visit", "form",
    "question"
  ) %in% names(dlu))) {
  ...
  if (!is.null(clu)) {
    ...
    if (!is.null(clu) && !all(clu_var_list %in% names(clu))) {
      stop(
        "Variable ", paste(setdiff(clu_var_list, names(clu)), collapse = ", "),
  ...
  if (check_catvar) {
    num_cols2 <- num_cols2[which(sapply(
      data[, num_cols2, with = FALSE],
      all_is_numeric
    ))]
  } # avoid none-numeric
  ...
  invisible(lapply(
    names(col_list),
    function(x) setattr(data[[x]], "label", col_list[[x]])
  ))
  ...
  if (!is.null(clu)) {
    ...
    if (any(is_empty(clu$catvalue))) {
      stop(
        "Variable ", paste(unique(clu$shortcode[is_empty(clu$catvalue)]),
          collapse = ", "
        ),
    ...
    invisible(lapply(
      names(clu_lst),
      function(x) setattr(data[[x]], "labels", clu_lst[[x]])
    ))
  }
...
#' Variable names, NOT the values, of the dlu data will be converted to lower
#'  cases.
#' @param x DLU data
...
#' This is will be used by \code{\link{apply_macro_dict}} to create variable
#'  label.
#'  \item{type}: type of the variable, it has IntegerData, Text, Date, Real and
#' Category four categories. This will be used by \code{\link{apply_macro_dict}}
#'  to convert variables to corresponding type.
#' }
#' @return A data.frame with `visit/form/question` column separated into
...
tidy_dlu <- function(x, clean_names = TRUE) {
  if_visit_form <- unique(sapply(
    (gregexpr("/", x[[2]], fixed = TRUE)),
    function(i) sum(i > 0)
  ))
...
#'
#' @description
#' \code{set_dlu} will set the provided DLU data to package environment, so it
#'  can be used for missing data report by \code{\link{cttab}}. It is user's
#'  responsibility to make sure values of the short code in the provided DLU
#'  data matches the variable names in the dataset. \code{set_dlu} will not have
#'   any effect on \code{\link{apply_macro_dict}}. Instead, the
#'   \code{\link{apply_macro_dict}} will override the `DLU` seetings done by
#' \code{set_dlu}.
#'
#' \code{get_dlu} can be used to get a copy of DLU data in stored by
#' \code{set_dlu}.
#'
...
#'
#' @description Extract data by form from MACRO dataset. Data will be
...
#' @param visit A character string or vector of visit name in the DLU file,
#' see \code{\link{tidy_dlu}}.
#' @param vars_keep Parameters to keep in the output data. This is useful if
...
extract_form <- function(data,
  ...
  if (!is.null(visit)) {
    if (!all(visit %in% dlu$visit)) {
      stop(
        "Visit name ", paste(setdiff(visit, dlu$visit), collapse = ", "),
        " can not be found in the DLU file."
      )
    }
  ...
  res <- lapply(unique(dlu$visit), function(v) {
    ...
    dt$if.all.miss <- rowSums(apply(
      dt[, dlu$question[dlu$visit == v], with = FALSE],
      2,
      function(x) x %in% c(NA, "")
    )) == length(vars_selc)
    dt$form_visit <- v
    dt
  })
