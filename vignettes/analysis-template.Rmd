...
### HTML and PDF
There is a function `write_quarto()` you can use to produce an HTML, DOCX or PDF file and this should cover main reporting formats. This function will use some predefined templates to build the report with [Quarto](https://cran.r-project.org/package=quarto). The parameters of this function works in a similar way of `write_docx()`. Although you can use this function to create DOCX file, you should stick to the `write_docx()` to get a pretty Word document, you can also convert this document to PDF with Word. 
```{r eval=FALSE}
write_quarto(
  report_title = "Vignette Report",
  author = "Simon Bond",
  filename = file.path("Output", "Reports", "Vignette_Report.html"),
  popn_labels = popn_labels,
  output_format = "html"
)
```
