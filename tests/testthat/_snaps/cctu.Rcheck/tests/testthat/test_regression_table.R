...
test_that("gls compare est, se and pvalue", {
  ...
  expect_warning(
    fit <- nlme::gls(
      twstrs ~ treat * rms::rcs(week, 3) + rms::rcs(twstrs0, 3) +
        rms::rcs(age, 4) * sex,
      data = both,
...
test_that("coxph regression", {
  ...
  se <- output[1:3, 2] %>%
    gsub(".*\\((.*)\\)", "\\1", .) %>%
    trimws()
  compare_with_rounding(se, X[, "se(coef)"]) %>%
    all() %>%
    expect_true()
  compare_with_rounding(output[1:3, 5], X[, "p"]) %>%
    all() %>%
    expect_true()
})
...
test_that("lme", {
  ...
  est <- output[, 2] %>%
    gsub("\\(.*\\)", "", .) %>%
    trimws()
  compare_with_rounding(est[1:2], X[, 2]) %>%
    all() %>%
    expect_true()
  se <- output[, 2] %>%
    gsub(".*\\((.*)\\)", "\\1", .) %>%
    trimws()
  compare_with_rounding(se[1:2], X[, 3]) %>%
    all() %>%
    expect_true()
  compare_with_rounding(est[5:7], Y[c(1, 2, 4), "vcov"]) %>%
    all() %>%
    expect_true()
  compare_with_rounding(output[5:7, 3], Y[c(1, 2, 4), "sdcor"]) %>%
    all() %>%
    expect_true()
...
test_that("gee", {
  ...
  est2 <- gee_mod[, 2] %>%
    gsub("\\(.*\\)", "", .) %>%
    trimws()
  compare_with_rounding(est1[1:6], est2[1:6]) %>%
    all() %>%
    expect_true()
})
