...
The code itself is assumed to be modularised into a sequence of code that calls other code using `source`.  Copies of the scripts used in this vignette are available within the package folder `/scripts`.

* `main.R` at the top with a minimal amount of initialisation: working directory, PATHs, `library(cctu)`, `run_batch()`, then a sequence of `source()`, final calling of `write_docx()` and maybe further calls to `render` for other outputs.
...
The legacy `create_word_xml()` path emitted a single XML document and applied `inst/extdata/xml_to_word.xslt` as the user-tweakable transform — at your own risk. The current `write_docx()` path is more involved: it ships a Word-compatible XSLT plus a `[Content_Types].xml` / `document.xml.rels` / `header.xml` / `footer.xml` / `core.xml` bundle under `inst/assets/`, and the docx zip needs them in lockstep. Please don't edit those files; if you need a different look, raise it with the package maintainer.
...
As described already `source()` captures a trail of which code sources other code. There is a demonstration below of how to convert this into a graphical tree representation using a combination of Rmarkdown and latex. Or you can just call `get_code_tree()` to grab a copy and write a local copy to a csv file say.
...
Need to create names for the population labels, including the number of subjects. Good to name the report ending with the suffix ".docx".
...
The output is [Vignette_Report_bak.docx](../doc/Vignette_Report_bak.docx).  

For the older function `create_word_xml()` output it was necessary to carry out post-processing in word. To permanently save, first go to file > Edit Links to Files;  highlight all the figures (shift + scroll), and click "break link". Then File> save as,  and ensure it is Saved As Type a "Word Document (*.docx)".
...
As an alternative to setting one `options(cctu_*)` at a time, `cctu_options()` collects all of the package's defaults (output folder, digits, p-value digits, render spec, figure formats, etc.) into one place:
...
`base::options()` still wins. Anything you have already set with `options(cctu_<name> = ...)` (typically in `.Rprofile` or at the top of `main.R`) takes precedence over `cctu_options()`, so existing scripts keep working unchanged. `cctu_options()` will warn if you set a value that is currently being shadowed by a base option.
