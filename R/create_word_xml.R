...
create_word_xml <- function(
  ...
  if( any( names(meta_table) != names(meta_table) %>% tolower)){
    warning("the variable names in meta_table supplied have been converted to lower case")
    names(meta_table) %<>% tolower
  }
