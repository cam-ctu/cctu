...
One can use fomula in the `cctab` just like `lm`, but you will not be able to group variables (described in the later section). The left hand side of the formula is the grouping variable, left blank if there isn't any. Right hand side of the fomula is the variables to be summarised in the table. The by visit variable should be proivded at the right hand side of the formula and separate with `|`. No `group` or `row_split` should be used in the formula interface. All the other parameters are the same.
```{r eval=FALSE}
# No grouping
X <- cttab( ~ AGE + SEX + BMIBL, data = df)
# Group summarise by ARM
X <- cttab(ARM ~ AGE + SEX + BMIBL, data = df)
# Group summarise by ARM and split by visit cycle
X <- cttab(ARM ~ AST + BILI | AVISIT, data = df)
