...
#' @return \code{var_lab} return variable label. If label doesn't exist
#' it return NULL . \code{var_lab<-} return variable (vector x) with attribute
#' "label" which equals submitted value.
#' @details Variable label is stored in attribute "label"
#' (\code{attr(x,"label")}). To drop variable label use
...
#'
#' @description These functions set/get/drop value labels. Duplicated values
...
`val_lab<-.default` <- function(x, value) {
  ...
  if (anyDuplicated(value)) {
    stop(
      "'val_lab' - duplicated values in labels: ",
...
#'
#' @param x Variable(s). Vector/data.frame/list.
#' @return \code{unlab} returns original variable x without variable label,
#' value labels.
...
#' @return \code{copy_lab} returns \code{new_var} with same variable label
