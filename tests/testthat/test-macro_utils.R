...
test_that("Apply DLU and CLU files", {
  ...
  expect_identical(names(cctu_env$dlu), tolower(c(
    "ShortCode", "Description", "Type",
    "Visit", "Form", "Question"
  )))
  ...
  expect_error(
    apply_macro_dict(dt, dlu = dlu, clu = clu, clean_names = FALSE),
  ...
  expect_error(
    apply_macro_dict(dt, dlu = dlu, clu = clu),
    "Variable arm has empty category value"
  )
...
test_that("Extract form", {
  ...
  expect_error(
    extract_form(df, "SCREENING"),
    "Form name SCREENING can not be found in the DLU file"
  )

  expect_error(
    extract_form(df, c("PatientReg", "Lab")),
  ...
  expect_error(
    extract_form(df, "Lab", visit = c("SCREENING", "TEST")),
