...
`geom_stepribbon` is an extension of the `geom_ribbon`, and is optimized
for Kaplan-Meier plots with pointwise confidence intervals or a
confidence band. The default `direction`-argument `"hv"` is appropriate
for right-continuous step functions like the hazard rates etc returned
by `pammtools`.
