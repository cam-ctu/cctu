...
test_that("alternative dimension for table", {
  ...
  var_lab(X$y) <- "Y>3"
  write_table(X,
    number = "1.1", directory = dir2,
    clean_up = FALSE
  )
  compare_file_text(
    file.path(dir1, "table_1.1.xml"),
    file.path(dir2, "table_1.1.xml")
  )
  class(X) <- c(class(X), "cttab")
  dir3 <- tempdir()
  write_table(X,
    number = "1.1", directory = dir3,
    clean_up = FALSE
  )
  compare_file_text(
    file.path(dir1, "table_1.1.xml"),
  ...
  X %>%
    dplyr::mutate(sex = factor(gender, labels = c("Male", "Female"))) %>%
    dplyr::group_by(sex) %>%
