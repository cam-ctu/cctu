...
#' @param check.names logical. If FALSE the names are preserved from \code{x},
#' but if TRUE they are converted to syntactically valid variable names,
#' which may not be desired for a table in a report.
#'
#' @return a data.frame or array that is \code{x} stacked vertically above
...
rbind_space <- function(x, y, check.names = FALSE) {# nolint: object_name_linter
  ...
  if (ncol(x) != ncol(y)) {
    stop("the number of columns do not match")
  }
