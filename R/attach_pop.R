#' Functions that attach and detach environments based on which populations are
#' used for a given table number
#'
#' @param number a character string, or number, giving the number of a table or
#' figure. Be careful if you want '2.10' rather than 2.1, say .
...
#' @param rm_envir logical, default=TRUE. Whether to run \code{\link{rm_envir}}
...
attach_pop <- function(number,
  ...
  if (length(popn_name) && popn_name != "" &&
    ...
    if (verbose) {
      cat(popn_name, "attached containing:", ls(popn_name), "\n")
    }
  } else {
...
detach_pop <- function(number,
  ...
  if (!is.null(popn_name) && popn_name %in% search()) {
    ...
    if (verbose) {
      cat(popn_name, "detached containing:", popn_list, "\n")
    }
  } else {
...
match_population <- function(number) {
  ...
  if (!("number" %in% names(meta_table))) {
    warning("Need to have 'number' column in meta_table")
  }
  if (!("population" %in% names(meta_table))) {
