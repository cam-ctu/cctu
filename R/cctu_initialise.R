...
#' @param output character string giving the name of the output folder. Can be overriden by setting the option("cctu_output").
...
rm_output <- function(output=getOption("cctu_output", default = "Output"),
  ...
  if( core){
    ...
    for( file in files){ file.remove(file.path(output,"Core",file))}
  }
  if( figures){
  ...
  if( reports){
    ...
    for( file in files){ file.remove(file.path(output,"Reports",file))}
  }
